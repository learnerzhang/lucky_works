...
def args():
    ...
    parser.add_argument('--mode', type=str, default='retrain', help='train|retrain|test|demo')
    parser.add_argument('--DEMO', type=str, default='tf_rnn_emergency', help='model for test and demo')
...
def re_train():
    ...
    with tf.compat.v1.Session(config=cfg()) as sess:
        ...
        textAttRNN.set_model_path(model_path=os.path.join(MODEL_PATH, FLAGS.DEMO))
...
def test():
    ...
    with tf.compat.v1.Session(config=cfg()) as sess:
        ...
        results = textRNN.predict(sess, inps)
        probs = textRNN.predict_prob(sess, inps)
    ...
    with tf.compat.v1.Session(config=cfg()) as sess:
        ...
        while True:
            ...
            if inp == '' or inp.isspace():
                ...
                inps = [inp.strip()]
                pred = textRNN.predict(sess, inps)[0]
                probs = textRNN.predict_prob(sess, inps)[0]
